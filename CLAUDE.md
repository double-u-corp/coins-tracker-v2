...
/src
  /pages
    ...
    /api
      ...
      cron-manual.ts                    # Manual cron trigger — session-cookie auth, no Home page UI (see changelog)
...
- **Client state**: `AuthProvider` + `useAuth()` (`features/auth/useAuth.tsx`) — a shared React Context wrapping the whole app in `_app.tsx`, fetching `/api/auth/me` once and holding ONE auth state every consumer reads. This used to be a plain hook (each call site fetched/held its own local state) until a real bug surfaced: `Layout` persists across navigation (Gotcha #1) so its state was set once before login and never updated, while other components that remounted after the post-login redirect picked up the new session fine — symptom was the nav's Buy/Sell + Manage Coins links staying hidden after login even though other authenticated-only UI elsewhere showed up correctly. `login()`/`logout()` called from anywhere (e.g. `LoginForm`) now update the shared state every consumer sees on its next render. See Gotcha #1a.
...
| `/api/cron-manual` | POST | session | Manual cron trigger — endpoint still works, no longer linked from Home UI |
...
- **Portfolio math is deliberately simple — 4 numbers, no average-cost/realized-vs-unrealized split** (`/api/transactions` `handleList`): `holdings = totalBought - totalSold`, `spent = totalPhpSpent (buys) - totalPhpReceived (sells)` (net PHP still invested — a sell's proceeds reduce this, they don't just sit in a separate "realized" bucket), `currentValue = currentPrice * holdings`, `gainLoss = currentValue - spent`. An earlier version of this schema tracked average buy price plus separate realized/unrealized gain-loss (FIFO/LIFO-adjacent accounting) — that was intentionally dropped in favor of this simpler 4-column view (`PortfolioEntry`: `holdings`, `spent`, `currentValue`, `gainLoss`). If asked to add back per-lot cost basis or realized/unrealized separation, that's a real product decision to confirm, not just a schema restoration — the simplification was requested, not accidental.
- **No per-unit price input anywhere in `/trade`, on purpose.** Both creating a transaction (`createTransactionSchema`) and correcting one (`updateTransactionSchema`, `PATCH /api/transactions`) always take `phpAmount` + `coinAmount` directly and derive `price = phpAmount / coinAmount` server-side — never the reverse. This was a deliberate product decision: the exact per-unit execution price isn't always known precisely, and asking for it (or worse, editing it independently of the two numbers the user actually knows for certain) let the three values drift out of sync. `Transaction.price` still exists in the schema and gets stored, but treat it as a derived/reference field only — don't add a UI path that lets it be set independently of `phpAmount`/`coinAmount` again.
...
- **`Record` storage is capped to one row per new daily extreme, not one row per cron run.** `runCronJob()` fetches every run (still 6x/day per coin) but only calls `prisma.record.create()` when the price is a new high or low *for that calendar day* (or the coin's first-ever observation) — a price that's "in the middle" of today's already-established range is fetched, used for all-time high/low comparison, and then NOT persisted. This is safe for all-time high/low accuracy: any all-time extreme is mathematically also a today's extreme, so it can never be silently skipped. The real tradeoff: Home's "Current Price" is drawn from the latest *stored* Record, so on a run where nothing new happens, it shows the last stored extreme rather than that exact instant's live price — accepted deliberately to cap storage growth (was writing 6 rows/day/coin regardless of whether the price actually moved).
- **`priceDirection` on `CoinSummary`** (`/api/coins` `handleSummary`) compares the two most recent stored Records per coin (`take: 2`, not `take: 1`) — `"up"`/`"down"`/`"flat"`, or `null` if there's fewer than two Records yet. Drives the ▲/▼ arrow next to Current Price on Home (`HomeTable.tsx`). Because of the day-based storage cap above, "previous" here means the last stored extreme, not literally the prior cron run — that's consistent with how "Current Price" already works, not a separate inconsistency.
- **Manual price update on Home** (`useHomeLogic.ts` → `submitPriceUpdate`, `POST /api/records`) replaced the "Run Cron Now" button — for coins added manually via Manage Coins (e.g. one with no live ticker pair) there was previously no way to update their price after the initial add. The coin dropdown defaults to the first monitored coin and has no blank "select a coin" option, gated behind login same as the other write actions. `/api/cron-manual` itself wasn't removed (see § Cron: how it works), just unlinked from Home's UI.
...
Both sources run per cron execution (`cronLogic.ts`), on both the scheduled `/api/cron` and a manual trigger via `/api/cron-manual` (no longer exposed as a Home page button — see changelog). RSS fetch failures are tracked separately from price-fetch errors (`newsWarnings`, not `errors`) and never affect the cron run's overall success/partial/error status — it's enrichment, not core functionality; a coin's price still gets fetched and recorded even if every RSS feed fails. `saveNewsItem` upserts on `externalId` when present (so a re-surfaced RSS article updates in place rather than duplicating, since feeds resurface recent items on every request) and just creates directly when absent (heuristic signals, nothing to dedupe against). RSS articles use `externalId: "rss:" + link`.

**Retention**: `NewsItem` rows older than `NEWS_RETENTION_DAYS` (30 days / 1 month, in `src/lib/retention.ts`) are pruned at the end of every cron run, same mechanism as `Record` pruning but with no "keep the newest" guard — nothing depends on an old signal surviving. Deliberately much shorter than `Record`'s 5-year window: signals are timely/contextual, not enduring reference data, and volume isn't capped by a fixed schedule the way `Record` is (a very newsworthy coin could otherwise accumulate rows indefinitely).
...
1a. **`useAuth()` must be called within `<AuthProvider>`** (wrapping everything in `_app.tsx`, outside `Layout`) — it throws otherwise. This follows directly from Gotcha #1: once `Layout` stopped remounting per page, a plain per-call-site `useAuth()` hook meant Layout's copy of the auth state went stale after a login/logout performed via a different component's `useAuth()` call (e.g. `LoginForm`). `AuthProvider` fixes this by being the single source of truth every `useAuth()` call reads from. Don't revert `useAuth.tsx` back to a standalone hook without re-solving this propagation problem some other way.
...
`/api/cron` itself doesn't know or care who's calling it — it's a thin authenticated HTTP wrapper that just checks the `Authorization` header and calls `runCronJob()`. That means swapping the trigger mechanism again later (e.g. [Upstash QStash](https://upstash.com/docs/qstash), [cron-job.org](https://cron-job.org), a real crontab entry on a self-hosted server) is purely a matter of what calls this URL on a schedule — no app code changes. `runCronJob()` in `src/lib/cronLogic.ts` is the actual logic (fetch prices, persist day-extreme Records only — see § Data storage below, generate news signals, prune old Records/NewsItem); `/api/cron-manual` calls that exact same function and still works for manual/testing triggers (e.g. via `curl`, or the GitHub Actions `workflow_dispatch` button), it's just no longer surfaced as a Home page button now that scheduled cron is live in production — see changelog.
...
13. Removed `vercel.json` entirely — GitHub Actions (`.github/workflows/cron.yml`) is now the sole cron trigger, not an alternative alongside Vercel's own Cron feature. Removed the CryptoPanic integration added in #12 — it went paid-only, and a free alternative was already the better long-term bet: added `fetchAllRssArticles`/`matchRssArticlesForCoin` in `newsApi.ts`, pulling from free public RSS feeds (Cointelegraph, CryptoSlate, NewsBTC) fetched once per cron run and matched per coin, with sentiment from a plain keyword-count heuristic (`classifySentiment`) since RSS carries no sentiment data of its own — this is now the app's real-article news source, needing zero configuration. Switched the whole app from SQLite to PostgreSQL (`datasource db { provider }` in `prisma/schema.prisma`) since Vercel's serverless filesystem can't persist a SQLite file — no in-repo automated data migration from the old SQLite database, just the schema/provider change.
14. Fixed a real auth bug: converted `useAuth` from a standalone per-call-site hook into a shared `AuthProvider` context (`useAuth.tsx`, wrapped around the app in `_app.tsx`) — symptom was the nav's Buy/Sell + Manage Coins links staying hidden after login even though other authenticated-only UI (Home's then-still-present "Run Cron Now" button) correctly showed up. Root cause: `Layout` persisting across navigation (Gotcha #1) meant its own `useAuth()` call never learned about a login performed via a different component's separate hook instance. See Gotcha #1a.
15. `/trade` — removed the Live/Manual price toggle and per-unit price input entirely, both for creating and correcting (`PATCH`) a transaction; every entry now always takes `phpAmount` + `coinAmount` directly, with `price` derived server-side purely for reference (see Key design decisions). Portfolio simplified from a 6-column average-cost/realized-vs-unrealized model down to 4 columns: Holdings, Total Spent (net of sell proceeds), Current Value, Gain/Loss. `Record` storage capped to one row per new daily high/low instead of one row per cron run (see Key design decisions — real tradeoff on Home's "Current Price" freshness). Added `priceDirection` to `CoinSummary` and a ▲/▼ arrow on Home's price column. Removed the "Run Cron Now" button from Home, replaced with an inline manual price-update control (`POST /api/records`) for coins with no live ticker feed — `/api/cron-manual` itself is untouched, just no longer linked from the UI. Calendar and the new Home price-update dropdown both default to the first monitored coin instead of a blank "select a coin" placeholder.
16. **This round**: shortened `NEWS_RETENTION_DAYS` from 180 to 30 (1 month) — the `<hr/>` run-separator in `NewsSection.tsx` was already in place from #12, no change needed there.
