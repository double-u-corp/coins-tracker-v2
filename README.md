...
- **Home** (`/`, public) — a card per monitored coin (Current Price with a ▲/▼ direction arrow, Recorded High/Low paired with Target High/Low, read-only) in a responsive grid, last-cron-run status banner, a dismissible banner when any coin hits its target, a modal that appears when a cron run sets a new all-time high/low, and a **Market Signals** section showing auto-generated bullish/bearish signals (see § News/Signals below — these are computed, not scraped news). Click a coin's **current price** to jump to its Calendar view. When logged in, click a coin's **name** instead to open a small modal, set its price manually (for coins with no live ticker feed), and — in the same modal — see and correct any of that coin's recent manual entries if one was typed wrong. Logged out, the name isn't a link, nothing to click (the price link works either way).
- **Calendar** (`/calendar`, public) — the first monitored coin is pre-selected (no blank "select a coin" step) unless you arrived via a `?symbol=` link from Home, in which case that coin is selected instead; daily high/low in a month grid.
...
That derived price **is** shown, though — each line in Transaction History includes it (e.g. "0.001 for ₱5,000 (₱5,000,000/coin)"), specifically so you can compare against past purchase prices when deciding whether a dip is worth buying. It just isn't something you can type in directly.
