...
export default function CalendarView() {
  ...
  // Calculate the absolute highest and lowest prices for the displayed month
  const monthHighest = recordsWithData.length > 0 
    ? Math.max(...recordsWithData.map((d) => d.high as number)) 
    : null;
    
  const monthLowest = recordsWithData.length > 0 
    ? Math.min(...recordsWithData.map((d) => d.low as number)) 
    : null;

  // Find the exact record objects for the high and low to extract their dates
  ...
  const volatilitySpread = monthHighest && monthLowest && monthLowest > 0 
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Monthly Summary Bar with Dates & Volatility Spread */}
      {!loading && !error && recordsWithData.length > 0 && (
        <div className="flex flex-col gap-4">
          ...
          {/* NEW: Actionable Volatility & Swing Guide */}
              ...
              <div className="space-y-2 text-sm">
                <div 
                  className={`flex items-baseline justify-between gap-2 rounded px-2 py-1.5 ${
                    isMonthHigh ? "bg-green-50 border border-green-200" : ""
                  }`}
                >
                  <span className={`text-xs font-medium uppercase tracking-wide flex items-center gap-1 ${
                    isMonthHigh ? "text-green-800 font-bold" : "text-gray-500"
                  }`}>
                    {isMonthHigh && <span>🏆</span>}
                    High
                  </span>
                  <span className={`break-all text-right font-mono ${
                    isMonthHigh ? "font-bold text-green-700" : "font-semibold text-green-600"
                  }`}>
                ...
                <div 
                  className={`flex items-baseline justify-between gap-2 rounded px-2 py-1.5 ${
                    isMonthLow ? "bg-red-50 border border-red-200" : ""
                  }`}
                >
                  <span className={`text-xs font-medium uppercase tracking-wide flex items-center gap-1 ${
                    isMonthLow ? "text-red-800 font-bold" : "text-gray-500"
                  }`}>
                    {isMonthLow && <span>📉</span>}
                    Low
                  </span>
                  <span className={`break-all text-right font-mono ${
                    isMonthLow ? "font-bold text-red-700" : "font-semibold text-red-600"
                  }`}>
